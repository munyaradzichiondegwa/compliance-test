...
export default function AppLayout() {
  ...
  return (
    <div className="min-h-screen flex bg-parchment">
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-64 bg-charcoal text-parchment flex flex-col transition-transform duration-200 ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex items-center gap-2.5 px-5 h-16 border-b border-white/10 shrink-0">
          ...
          <div className="leading-tight">
            ...
            <div className="text-[10px] text-gold-light uppercase tracking-wider">Institutional Portal</div>
          </div>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2 rounded text-sm transition-colors ${
                  isActive ? 'bg-gold text-white font-medium' : 'text-parchment/75 hover:bg-white/10 hover:text-parchment'
                }`
              }
            >
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.slice(0, 20).map((n) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 border-b border-line last:border-0 hover:bg-parchment/60 ${!n.is_read ? 'bg-gold/5' : ''}`}
                      >
                        <div className="flex items-start gap-2">
                          {!n.is_read && <span className="w-1.5 h-1.5 rounded-full bg-gold mt-1.5 shrink-0" />}
...
export function SealMark({ size = 32 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" className="shrink-0">
      <circle cx="32" cy="32" r="30" fill="#161512" />
      <circle cx="32" cy="32" r="30" fill="none" stroke="#D9A62E" strokeWidth="2" />
      <path d="M32 14 L44 19 V31 C44 40 38.5 46.5 32 50 C25.5 46.5 20 40 20 31 V19 Z" fill="#F7F4EC" />
      <path d="M25.5 32.5 L30 37 L39 27" fill="none" stroke="#161512" strokeWidth="3.4" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
