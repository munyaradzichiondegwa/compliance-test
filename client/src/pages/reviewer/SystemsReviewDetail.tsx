  ...
  return (
    <div>
      ...
      <div className="grid lg:grid-cols-3 gap-4 mb-6">
        {STATUS_FLOW.map((s, i) => (
          <div key={s} className={`text-center py-2 rounded text-xs font-medium ${i < currentIdx ? 'bg-status-green-bg text-status-green' : i === currentIdx ? 'bg-gold text-white' : 'bg-line text-slate'}`}>
            {s}
          </div>
        ))}
      </div>
      ...
      {data.ai_summary && (
        <Card className="mb-6 bg-charcoal/[0.03]">
