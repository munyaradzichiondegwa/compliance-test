      ...
      <div className="flex gap-2 mb-5">
        {['', 'Draft', 'Submitted', 'UnderReview', 'Returned', 'Approved', 'Closed'].map((s) => (
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${statusFilter === s ? 'bg-charcoal text-white border-charcoal' : 'border-line text-slate hover:border-charcoal/40'}`}
          >
            {s || 'All'}
          </button>
        ))}
              ...
              {assessments.map((a) => (
                <tr key={a.id} className="hover:bg-parchment/60 cursor-pointer" onClick={() => navigate(`/app/assessments/${a.id}`)}>
                  <td className="px-4 py-3">
                    <span className="text-charcoal font-medium">{a.institution_name}</span>
