      ...
      <div className="min-h-screen bg-navy flex items-center justify-center p-4">
        <div className="card max-w-md w-full p-8 text-center">
          ...
          <div className="bg-parchment border border-line rounded-lg p-4 flex items-center justify-between gap-3 mb-5">
            <span className="font-mono text-lg tracking-wider text-navy">{trackingCode}</span>
            <button
              onClick={() => {
  ...
  return (
    <div className="min-h-screen bg-parchment">
      ...
      <main className="max-w-3xl mx-auto px-5 py-10">
        <div className="card p-4 mb-6 flex items-start gap-3 bg-navy text-parchment border-none">
          <Lock size={18} className="text-brass-light shrink-0 mt-0.5" />
          <div className="text-xs leading-relaxed">
            <strong className="text-brass-light">This report is encrypted in your browser before it is sent.</strong> We
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {CATEGORIES.map((c) => (
                <button
                  ...
                  className={`text-xs px-3 py-2 rounded border transition-colors text-left ${category === c ? 'border-brass bg-brass/10 text-brass-dark font-medium' : 'border-line hover:border-brass/50'}`}
                >
                  {c}
                </button>
              ))}
