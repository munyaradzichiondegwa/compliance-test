    ...
    <div>
      ...
      <div className="flex gap-2 mb-6">
        {[['templates', 'Templates'], ['email', 'Email Outbox'], ['sms', 'SMS Outbox']].map(([k, l]) => (
          <button key={k} onClick={() => setTab(k as any)} className={`text-xs px-3 py-1.5 rounded-full border ${tab === k ? 'bg-navy text-white border-navy' : 'border-line text-slate'}`}>{l}</button>
        ))}
      </div>
          ...
          {templates.map((t) => (
            <Card key={t.id}>
              <div className="flex items-center justify-between mb-2">
                <span className="font-mono text-xs text-brass-dark">{t.code}</span>
